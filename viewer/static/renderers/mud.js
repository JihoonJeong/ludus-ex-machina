...
(function () {
    ...
    function mapSvg(cur, view, codes) {
        ...
        for (const r of drawn) {
            ...
            nodes.push(
                `<g>${isCur ? `<rect x="${x - 3}" y="${y - 3}" width="${NW + 6}" height="${NH + 6}" rx="6" fill="none" stroke="${C.you}" stroke-width="1" opacity="0.45"/>` : ''}` +
                ...
                `${isCur ? `<text x="${cx(r)}" y="${y - 5}" text-anchor="middle" font-size="11" fill="${C.you}">★</text>` : ''}</g>`
            );
        }
