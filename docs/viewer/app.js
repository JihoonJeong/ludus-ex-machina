...
async function loadLeaderboard() {
    ...
    leaderboardData = apiLeaderboard || await dataSource.getLeaderboard();
...
async function loadMatch(matchId) {
    const bundle = await dataSource.getMatchBundle(matchId);

    if (!bundle?.config || !bundle?.log) {
        alert('Failed to load match data');
        navigateTo(null);
        return;
    }

    const { config, log, result } = bundle;
...
async function initMultiCell(matchInfo, cellEl) {
    ...
    const bundle = await dataSource.getMatchBundle(matchId);
    if (!bundle?.config || !bundle?.log) return;
    const { config, log, result } = bundle;
...
document.addEventListener('DOMContentLoaded', async () => {
    // Initialize data source (static vs server)
    await dataSource.init();

    // Hide server-only features in static mode
    if (dataSource.isStatic) {
        document.querySelectorAll('.server-only').forEach(el => el.style.display = 'none');
    }
